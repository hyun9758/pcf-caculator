...
| 단계 | 소요 | 내용 |
|------|------|------|
| 프로젝트 셋업 | ~15분 | Next.js + Tailwind + shadcn/ui + Nivo 설치 |
| 도메인 모델링 | ~20분 | 타입 정의, 상수, 배출계수 데이터 설계 |
| Mock 데이터 생성 | ~25분 | 5개 제품, 28개 배출계수, 40개 활동 데이터 (공개 자료 참조) |
| 계산 엔진 + API | ~20분 | PCF 계산 로직 + REST API 10개 |
| 대시보드 페이지 | ~25분 | KPI, Scope 도넛, 추세 라인, 순위 바, 뷰 토글 |
| 제품 목록 + 상세 | ~25분 | 필터/정렬, Sankey 다이어그램, 활동 데이터 테이블 |
| 데이터 입력 CRUD | ~20분 | 추가/삭제 폼, 배출계수 드롭다운, 실시간 프리뷰 |
| 제품 비교 + 시뮬레이션 | ~25분 | 4개 제품 비교, 4가지 감축 시나리오 |
| 반응형 + 폴리시 | ~15분 | 모바일 Sidebar, 404, 로딩 스켈레톤 |
| Prisma + Neon 연동 | ~20분 | 스키마, 마이그레이션, 시드, Repository 패턴 |
| 문서 + 배포 | ~10분 | README, 설계 문서, Vercel 배포 |

**시간이 많이 소요된 부분:**
- **Mock 데이터 설계**: ecoinvent, IPCC 등 공개 자료에서 사실적인 배출계수 값을 찾아 교차 검증
- **Sankey 다이어그램**: Nivo Sankey 데이터 구조를 PCF 전과정에 맞게 3단계(소스→단계→총합)로 변환
- **Prisma 7 호환성**: Prisma 7의 adapter 기반 구조에 맞춰 PrismaPg 설정 + Neon SSL 연결
...
|탄소 배출 대시보드|
|-------|
|<img width="1877" height="1016" alt="image" src="https://github.com/user-attachments/assets/c978102f-3ecb-4107-9e0d-fbdc15633e1f" />|
...
|제품 PCF 상세|
|-------|
|<img width="1901" height="1004" alt="image" src="https://github.com/user-attachments/assets/b6230ca6-d7b6-487a-b4c4-069bca344a32" />|
...
|데이터 입력|
|-------|
|<img width="1902" height="987" alt="image" src="https://github.com/user-attachments/assets/382ed00d-1aaa-4671-a253-107014ea1005" />|
...
|제품 비교|
|-------|
|<img width="1898" height="1018" alt="image" src="https://github.com/user-attachments/assets/90457764-2645-4197-8584-0b707d32a7c1" />|
...
|감축 시뮬레이션|
|-------|
|<img width="1906" height="1001" alt="image" src="https://github.com/user-attachments/assets/a6f93794-495e-4f99-8325-deb8ea3af7c5" />|


### 6. Excel 임포트 (보너스)
- 과제 제공 Excel 파일 드래그&드롭 업로드
- "과제용 데이터" 시트 자동 인식 + 파싱
- 활동 유형/배출계수 자동 매칭

### 7. AI 탄소 분석 (`/products/[id]`)
- Gemini AI 기반 PCF 분석 (제품 상세 페이지 하단)
- 배출 원인 분석 + 감축 우선순위 제안
...
│ product_id  (FK)     │
│ date                 │
│ activity_type        │  ← 전기 / 원소재 / 운송
│ lifecycle_stage      │  ← manufacturing / raw_material / transportation
...
계산 결과(PcfResult)는 DB에 저장하지 않음 — 매번 실시간 계산.
CO₂e = quantity × co2e_per_unit
```
...
### 과제 데이터: 컴퓨터 화면 (CT-045)

**활동 데이터 31건** (2025-01 ~ 2025-08):

| 활동 유형 | 건수 | 단위 | GHG Scope |
|----------|------|------|-----------|
| 전기 (한국전력) | 10건 | kWh | Scope 2 |
| 원소재 (플라스틱 1, 2) | 12건 | kg | Scope 3 |
| 운송 (트럭) | 9건 | ton-km | Scope 3 |
...
| POST | `/api/import` | Excel 파일 임포트 (multipart/form-data) |
| POST | `/api/ai/analyze` | AI PCF 분석 (Gemini) |
